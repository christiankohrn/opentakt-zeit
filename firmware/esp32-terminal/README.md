...
V1 bucht **nur mit WLAN**. Ohne RFID: BOOT-Taste oder Serial `TAP`.
...
Port setzen z. B. `pio run -t upload --upload-port COM5`.

## Erster Start

1. Kein WLAN gespeichert → Access Point `opentakt-XXXX`, Passwort auf dem Display (`ot-…`).
2. Handy verbinden, Browser `http://192.168.4.1`.
3. SSID, Passwort, Server (`https://zeit.firma.de` oder `http://192.168.1.10:8000`), Secret (`esp_terminal_secret`).
